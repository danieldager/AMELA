...
class LSTMConfig(PretrainedConfig):
    ...
    def __init__(
        self,
        vocab_size: int = 2002,  # 2000 audio tokens + SOS + PAD
...
class LSTM(PreTrainedModel):
    ...
    def forward(
        ...
        Args:
            ...
            labels: Target token IDs for loss calculation (same format as input_ids)
        ...
        if lengths is not None:
            ...
            lstm_output, _ = pad_packed_sequence(
                packed_output,
                batch_first=True,
                total_length=input_ids.size(1),  # Ensure same length as input
            )
        else:
    ...
    def generate(
        ...
        Args:
            input_ids: Starting tokens [batch_size, seq_len] (typically just SOS token)
        ...
        h0 = torch.zeros(
            self.config.num_layers, batch_size, self.config.hidden_size, device=device
        )
        c0 = torch.zeros(
            self.config.num_layers, batch_size, self.config.hidden_size, device=device
        )

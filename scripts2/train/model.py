...
def print_training_summary(config, args, model):
    ...
    {'='*60}
    MODEL ARCHITECTURE:
    - Layers: {config.n_layer} | Heads: {config.n_head} | Embed: {config.n_embd}
        ...
        if os.environ.get("RANK", "0") == "0":
            if logs is not None and ("loss" in logs or "eval_loss" in logs):
                ...
                if "loss" in logs:
                    ...
                    print(
                        f"Step: {steps_done:5d} | Loss: {logs['loss']:.4f} | "
...
if __name__ == "__main__":
    train_dir = "/scratch2/ddager/amela/tokens/chunks30_mhubert/"
    eval_dir = "/scratch2/ddager/amela/tokens/chunks-eval_mhubert/"
    ...
    training_args = TrainingArguments(
        output_dir="./checkpoints/gpt2_test_chunks30",
